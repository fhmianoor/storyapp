import { saveStory, getStoryById, deleteStory } from '../utils/idb.js';
...
export default class DetailStoryView {
  async render() {
    return `
      <main id="main-content" tabindex="-1">        
        ...
        <div id="save-actions-container" class="center-btn-container"></div>    
      </main>`;
  }
  ...
  async renderDetail(story) {
    ...
    // 🔐 Validasi null / struktur tidak lengkap
    if (!story || typeof story !== 'object' || !story.id || !story.photoUrl) {
      console.warn('Data cerita tidak valid:', story);
      container.innerHTML = '<p>Story tidak ditemukan atau data tidak lengkap.</p>';
      return;
    }
    ...
    container.innerHTML = `
      <div class="story-detail">
        <img 
          ...
          style="width:100%;max-width:400px;object-fit:cover;border-radius:8px;box-shadow:0 2px 8px #5BC0EB22;" 
  ...
  async renderSaveButton() {
    ...
    if (!this._currentStory || !this._currentStory.id) {
      container.innerHTML = '';
      return;
    }
    ...
    try {
      ...
      if (isSaved) {
        container.innerHTML = `
          <button id="report-detail-delete" class="delete-btn">
            <i class="fa-solid fa-trash"></i> Hapus Cerita
          </button>
        `;
        ...
        container.innerHTML = `
          <button id="report-detail-save" class="save-btn">
            <i class="fa-solid fa-floppy-disk"></i> Simpan Cerita
          </button>
        `;
      ...
      console.error('Gagal memproses IndexedDB:', error);
      container.innerHTML = '<p>Terjadi kesalahan lokal saat menyimpan data.</p>';
    }
  }
}
